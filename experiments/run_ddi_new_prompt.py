...
def create_demo(fst12_ben, fst12_mal, fst56_ben, fst56_mal, filter_rare = False, random_seed=141):
    """
    Create demo examples for the given skin tones and disease classes.

    Args:
        fst12_ben (int): Number of benign examples for skin tone 12.
        fst12_mal (int): Number of malignant examples for skin tone 12.
        fst56_ben (int): Number of benign examples for skin tone 56.
        fst56_mal (int): Number of malignant examples for skin tone 56.
        filter_rare (bool, optional): Whether to filter out rare diseases. Defaults to False.
        random_seed (int, optional): Random seed for sampling. Defaults to 141.

    Returns:
        pd.DataFrame: Demo examples.
    """
...
Question: Based on the visual characteristics presented in the image, would you classify the skin lesion as likely:
...
"""
        ...
        for idx, i in enumerate(test_df.iloc[start_idx:end_idx].itertuples()):
            ...
            prompt += f"""<<IMG>>Given the image above, answer the following question using the specified format. 
Question {qn_idx}: Based on the visual characteristics presented in the image, would you classify the skin lesion as likely:
...
Do not deviate from the above format. Repeat the format template for the answer."""
    ...
    results["token_usage"] = total_usage
    with open(f"./ddi_results_prompt_1/{EXP_NAME}.pkl", "wb") as f:
        pickle.dump(results, f)
