...
# Constants
SEEDS = [141, 10, 100]
MODELS = ["Gemini1.5", "gpt-4o-2024-05-13", "claude"]
NUM_QNS_PER_ROUND = 50


def create_demo(female_ben, female_mal, male_ben, male_mal, random_seed=141):
    """
    Load demo example frame and choose relevant demo examples based on demographic composition.
...
Do not deviate from the above format. Repeat the format template for the answer."""
    ...
    results["token_usage"] = total_usage
    with open(
        f"/home/groups/roxanad/sonnet/icl/ManyICL/ManyICL/chexpert_results_rebuttal/{EXP_NAME}.pkl",
        "wb",
    ) as f:
        pickle.dump(results, f)
...
if __name__ == "__main__":
    for model in MODELS:
        for seed in SEEDS:
            ...
            # # base rate
            main(model,
                40, 0, 40, 0, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model,
                30, 10, 30, 10, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model,
                20, 20, 20, 20, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model,
                10, 30, 10, 30, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model,
                0, 40, 0, 40, NUM_QNS_PER_ROUND, random_seed=seed)

            # inverted base rate

            main(model, 0, 40, 40, 0, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model, 10, 30, 30, 10, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model, 20, 20, 20, 20, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model, 30, 10, 10, 30, NUM_QNS_PER_ROUND, random_seed=seed)

            main(model, 40, 0, 0, 40, NUM_QNS_PER_ROUND, random_seed=seed)